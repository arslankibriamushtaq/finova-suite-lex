...
const sullisCash: ModuleLocale = {
  ...
  en: {
    ...
    "loans.subtitle": "Every cash advance taken, with its live payoff and the terms it was priced at.",
    ...
    "loans.readOnlyNote": "The loan book is read-only — there is no manual settle, waive or cancel. Penalty and payoff on an open loan are computed as of now.",
    "loans.stats.totalLoans": "{{count}} loan",
    "loans.stats.totalLoans_other": "{{count}} loans",
    "loans.stats.outstanding": "Outstanding principal (active + overdue)",
    ...
    "loans.detail.snapshotNote": "These rates are the loan's own snapshot, taken at disbursement. Changing the product's terms never re-prices a loan a customer already took.",
    ...
    "loans.detail.walletPurged": "This wallet has been purged, so only its ID remains. The loan itself is never hidden.",
  ...
  fr: {
    ...
    "loans.subtitle": "Chaque avance accordée, son solde de rachat actuel et les conditions appliquées.",
    ...
    "loans.readOnlyNote": "Le registre des prêts est en lecture seule — ni règlement, ni remise, ni annulation manuelle. La pénalité et le rachat d'un prêt ouvert sont calculés à l'instant.",
    "loans.stats.totalLoans": "{{count}} prêt",
    "loans.stats.totalLoans_other": "{{count}} prêts",
    "loans.stats.outstanding": "Principal restant dû (actifs + en retard)",
    ...
    "loans.detail.snapshotNote": "Ces taux sont ceux figés au décaissement. Modifier les conditions du produit ne retarife jamais un prêt déjà accordé.",
    ...
    "loans.detail.walletPurged": "Ce portefeuille a été purgé : seul son identifiant subsiste. Le prêt reste toujours visible.",
  ...
  ar: {
    ...
    "loans.readOnlyNote": "سجل التمويلات للقراءة فقط — لا تسوية ولا إعفاء ولا إلغاء يدوي. تُحتسب الغرامة ومبلغ السداد للتمويل المفتوح في اللحظة الحالية.",
    "loans.stats.totalLoans": "{{count}} تمويل",
    "loans.stats.totalLoans_other": "{{count}} تمويلًا",
    "loans.stats.outstanding": "أصل المبلغ القائم (النشط والمتأخر)",
    ...
    "loans.detail.snapshotNote": "هذه المعدلات مثبّتة وقت الصرف. تغيير شروط المنتج لا يعيد تسعير تمويل حصل عليه العميل بالفعل.",
