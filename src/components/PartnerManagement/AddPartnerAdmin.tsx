  ...
  const handleSubmit = async () => {
    try {
      ...
      if (response?.data?.success) {
        toast.success(response?.data?.message || "Partner admin added successfully");
        navigate(`/PartnerManagement/PartnerAdminList?id=${partnerId}`);
      } else {
        toast.error(response?.data?.message || "Failed to add partner admin");
      }
    } catch (error: any) {
      console.error("Error adding partner admin:", error);
      toast.error(error?.response?.data?.message || "Failed to add partner admin");
    } finally {
      setLoading(false);
    }
  };
