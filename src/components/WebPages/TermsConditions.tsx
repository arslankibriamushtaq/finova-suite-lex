...
const TermsConditions: React.FC = () => {
  ...
  const fetchPageData = async (currentLocale: string = locale) => {
    try {
      ...
      if (result.success && result.data && result.data.page && result.data.page.sections) {
        ...
        const currentBreadcrumb = breadcrumbs.find((b: any) => !b.url) || { name: 'Terms & Conditions' };
        ...
        const parsedData: TermsConditionsData = {
          ...
          hero: {
            image: termsHeroSection.hero_image?.url || ''
          },
          breadcrumb: {
            home: { 
      ...
      if (err instanceof TypeError && err.message.includes('Failed to fetch')) {
        ...
        const fallbackData: TermsConditionsData = {
          ...
          breadcrumb: {
            home: { text: 'Home', url: '/' },
            terms: { text: 'Terms & Conditions', url: '/terms' }
          },
          content: {
            heading: 'N/A',
            description: 'N/A'
  ...
  return (
    <div className="header-footer-settings__landing-template page-terms">
      {/* Global Header */}
      ...
      <div style={{
        ...
        <span style={{ color: '#7f8c8d' }}>
          {pageData.breadcrumb.terms.text}
        </span>
      </div>
     </div>
      {/* Main Content */}
