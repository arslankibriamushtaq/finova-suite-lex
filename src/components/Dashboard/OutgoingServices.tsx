...
const OutgoingServices = (props: any) => {
  ...
  const barChartOptions = {
    xAxis: {
      type: "category",
      data: props?.dashboardData?.services_report_card?.graph?.map(
        (day) => day?.name
      ), // Days in Feb 2025
    ...
    series: [
      {
        name: t("ibft.tab.totalFunds"), // Name for the first set of bars
        data: props?.dashboardData?.services_report_card?.graph?.map(
          (day) => day?.total_fund
  ...
  return (
    <div className="service">
      ...
      >
        ...
        >
          <h4>{t("outgoingServices.title")}</h4>
...
export default OutgoingServices;
