...
const BusTickets = (props: any) => {
  ...
  const numbersBarChartOptions = {
    xAxis: {
      type: "category",
      data: props?.dashboardData?.bus_tickets?.graph?.map((day) => day?.name), // Days in Feb 2025
      nameLocation: "middle", // Position the label in the middle
      nameGap: 30, // Increase the gap between the label and the axis
    },
    yAxis: {
    ...
    series: [
      {
        name: "Series 1", // Name for the first set of bars
        data: props?.dashboardData?.bus_tickets?.graph?.map(
          (day) => day?.amount
        ...
        itemStyle: {
          color: "#AB1920", // Color for the first set of bars
        },
      },
    ],
    tooltip: {
