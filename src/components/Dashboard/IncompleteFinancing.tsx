...
import {
  getAllApprovedFinancingApplications,
  getAllFinancingApplications,
  getAllIncompleteFinancingApplications,
  getAllRejectedFinancingApplications,
  resendLoginEmail,
...
const IncompleteFinancing = () => {
  ...
  const getLeadsList = async () => {
    try {
      setSkelitonLoading(true);
      const response = await getAllIncompleteFinancingApplications(page, pageSize);
  ...
  const mappedData =
    ...
    data?.map((item: any) => {
      return {
        ...
        status: item?.status || getStatusText(item?.status_id),
...
export default IncompleteFinancing;
