...
import { usePermissions, LOV_NET_WORTH_RANGE_PERMISSIONS } from "../../hooks/useProductPermissions";

const NetWorthRanges = () => {
  const { t } = useTranslation("lov");
  const { hasPermission } = usePermissions();
  const canCreate = hasPermission(LOV_NET_WORTH_RANGE_PERMISSIONS.CREATE);
  const canEdit = hasPermission(LOV_NET_WORTH_RANGE_PERMISSIONS.EDIT);
  const canDelete = hasPermission(LOV_NET_WORTH_RANGE_PERMISSIONS.DELETE);
