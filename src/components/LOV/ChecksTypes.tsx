...
const ChecksTypes = () => {
  ...
  const canCreateCheck = hasPermission(CHECKS_TYPES_PERMISSIONS.CREATE);
  const canEditCheck = hasPermission(CHECKS_TYPES_PERMISSIONS.EDIT);
  const canDeleteCheck = hasPermission(CHECKS_TYPES_PERMISSIONS.DELETE);
  const canUpdateStatus = hasPermission(CHECKS_TYPES_PERMISSIONS.UPDATE_STATUS);
  
  // Check if any action is available
  const hasAnyAction = canEditCheck || canDeleteCheck;
      ...
      cell: (row: any) => (
        <Switch
          ...
          onChange={async (checked) => {
            ...
            try {
              const res = await updateCheckTypesStatus(row.id, body);
              if (res) {
