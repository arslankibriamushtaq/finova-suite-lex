...
const IndividualRelation = () => {
  const [editRowId, setEditRowId] = useState(null);
  const [modal, setModal] = useState(false);
  const [allOccupation, setAllOccupation] = useState<any>();
  ...
  const occupationId = async (id: any) => {
    try {
      const res = await getAllOccupationById(id);
      if (res) {
  ...
  const handleSubmit = async (formField: any) => {
    try {
      ...
      if (modal && editRowId) {
        // body["id"] = editRowData;
        body.id = editRowId;
        res = await updateOccupation(body);
      } else {
        res = await createOccupation(body);
      }
  ...
  const handleDelete = async (row: any) => {
    try {
      const res = await deleteOccupation(row.id);
      if (res) {
        toast.success(res.data.notificationMessage);
        getOccupation();
      }
  ...
  const mappedData =
    allOccupation &&
    allOccupation.map((item: any, index: any) => {
      return {
        id: item.id,
        index: index + 1,
        name: item.name,
                  ...
                  <Modal.Body>
                    ...
                    <div className="d-flex mt-4 justify-content-center ">
                      <button className="theme-btn-next" type="submit">
                        {editRowId ? "Update occupatiion" : "Add occupation  "}
                      </button>
                    </div>
                  </Modal.Body>
                </Form>
