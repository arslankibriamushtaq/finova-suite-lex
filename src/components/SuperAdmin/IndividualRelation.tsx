...
const IndividualRelation = () => {
  ...
  const relationId = async (id: any) => {
    try {
      const res = await getAllRealationsById(id);
      if (res) {
  ...
  const handleSubmit = async (formField: any) => {
    try {
      ...
      if (modal && editRowId) {
        // body["id"] = editRowData;
        body.id = editRowId;
        res = await updateAllRelations(body);
      } else {
        res = await createAllRelations(body);
      }
  ...
  const handleDelete = async (row: any) => {
    try {
      const res = await deleteRelation(row.id);
      if (res) {
        toast.success(res.data.notificationMessage);
        getAllRelation();
      }
                  ...
                  <Modal.Body>
                    ...
                    <div className="d-flex mt-4 justify-content-center ">
                      <button className="theme-btn-next" type="submit">
                        {editRowId ? "Update Relation" : "Add Relation"}
                      </button>
                    </div>
                  </Modal.Body>
                </Form>
