...
const AdminReason = () => {
  ...
  const getAllCallReason = async () => {
    try {
      const res = await getAllReasons(page, pageSize);
      if (res) {
  ...
  const callReasonId = async (id: any) => {
    try {
      const res = await getCallReasonById(id);
      if (res) {
  ...
  const handleSubmit = async (formField: any) => {
    try {
      ...
      if (modal && editRowId) {
        ...
        res = await updateCallReason(body);
      } else {
        res = await createCallReason(body);
      }
  ...
  const handleDelete = async (row: any) => {
    try {
      const res = await deleteCallReason(row.id);
      if (res) {
        toast.success(res.data.notificationMessage);
        getAllCallReason();
      }
      ...
      <div className="cs-table p-4">
        ...
        >
          <ModalHeader closeButton>
            <h3>Add Call Reason</h3>
          </ModalHeader>

          <Formik
            initialValues={{
              description: editRowId ? updateData?.description : "",
              id: updateData?.id || "",
            }}
                  ...
                  <Modal.Body>
                    ...
                    <div className="d-flex mt-4 justify-content-center ">
                      <button className="theme-btn-next" type="submit">
                        {editRowId ? "Update Call Reason" : "Add Call Reason"}
                      </button>
                    </div>
                  </Modal.Body>
                </Form>
