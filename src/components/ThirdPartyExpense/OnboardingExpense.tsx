...
const OnboardingExpense = ({handleOpen,refreshData}: any) => {
  ...
  const handleGetData = async () => {
    try {
      ...
      const res = await GetAllThirdPartyExpense(page,pageSize,'Onboarding');
      if (res?.data?.success) {
        const data = res.data.data;
        const newdata= data.filter((item: any)=>item.expenseCategory==='Onboarding' || item.expenseCategory==='Both')
        setAllCallActivity(newdata || []);
