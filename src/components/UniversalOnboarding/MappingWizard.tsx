    ...
    "`}<span className="text-white underline decoration-amber-500/50 underline-offset-4">id_number</span>{`": "string"
             ...
             <div className="space-y-4 pt-4">
                {[
                  ...
                  <div key={col.name} className={`flex justify-between items-center p-3 rounded-xl border ${
                    col.active ? 'bg-emerald-50 border-emerald-100 text-emerald-900' : 'bg-slate-50 border-slate-100 text-slate-400'
      ...
      <div className="mt-16 space-y-4">
        <div className="flex items-center justify-between mb-6">
          ...
          <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">3 Active Rules</span>
        </div>
        ...
        {mappings.map((m, i) => (
          ...
          >
            ...
            <div className={`flex items-center gap-2 px-4 py-2 rounded-2xl ${
              m.status === 'mapped' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
            }`}>
              {m.status === 'mapped' ? (
