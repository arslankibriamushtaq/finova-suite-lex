...
const EditEarlySettlement: any = ({ productId, setSelectedTab }: any) => {
        ...
        .then((res) => {
          if (res.status == 200) {
            if (res.data.notificationMessage == "Operation successful.") {
              setFormValues({
                fromDay: res?.data?.data?.delinquency[0].fromDay,
                tillDay: res?.data?.data?.delinquency[0].tillDay,
              });
            } else {
...
export default EditEarlySettlement;
