...
import { createDefaultPayment, getDelinquencyByStatus, updateDefaultPayment, deleteDefaultPayment } from "../../../redux/apis/apisCrudLms";
import Loader from "../../Loader/Loader";

const Default = ({ productId, setSelectedTab }: any) => {
  ...
  const confirmDelete = async () => {
    ...
    setLoader(true);
    try {
      const res = await deleteDefaultPayment(sectionIds[deletingIndex]);
  ...
  const handleUpdateSection = async () => {
    ...
    try {
      ...
      const res = await updateDefaultPayment(payload);
      if (res?.data?.notificationMessage === "Operation successful." || res?.data?.success) {
  ...
  const updateSubmitForm = async () => {
    ...
    try {
      ...
      const res = await createDefaultPayment(payload);
  ...
  const getDeliquencyData = async () => {
    ...
    try {
      setLoader(true);
      const res = await getDelinquencyByStatus(0, productId); // 0 = Default Payment
  ...
  return (
    <div>
      ...
      <div className="mt-4 mb-3">
        ...
        >
          <span>Default Payment Settings</span>
          <button
