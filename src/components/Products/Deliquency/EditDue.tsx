...
const EditDue: any = ({ productId, setSelectedTab }: any) => {
        ...
        .then((res) => {
          if (res.status == 200) {
            if (res.data.notificationMessage == "Operation successful.") {
              setFormValues({
                fromDay: res?.data?.data?.delinquency[1].fromDay,
                tillDay: res?.data?.data?.delinquency[1].tillDay,
                penalty: res?.data?.data?.delinquency[1].penaltyAmount
                  ? res?.data?.data?.delinquency[1].penaltyAmount
                  : res?.data?.data?.delinquency[1].penaltyPercentage,
              });
              setradioInputValue(
