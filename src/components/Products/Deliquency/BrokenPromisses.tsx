...
const BrokenPromisses = ({ productId, setSelectedTab }: any) => {
  ...
  const getDeliquencyData = async () => {
    ...
    try {
      ...
      if (res.data.notificationMessage == "Operation successful.") {
        setradioInputValue(
          res?.data?.data.delinquencies[5]?.isPercentage
            ? "Percentage"
            : "Fixed"
        );
       const data = res?.data?.data?.delinquencies.find((item: { delinquencyType: number; })=>item.delinquencyType === 6);
        ...
        setFormValues({
          ...
          penalty: data?.isPercentage
            ...
            : data?.penaltyAmount,
          promisesPerYear: data?.promisesPerYear,
          promisesPerLoan: data?.promisesPerLoan,
        });
        setLoader(false);
      }
    } catch (error: any) {
...
export default BrokenPromisses;
