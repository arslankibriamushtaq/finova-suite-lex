        ...
        .then((res) => {
          if (res.status == 200) {
            if (res.data.notificationMessage == "Operation successful.") {
              setFormValues({
                fromDay: res?.data?.data?.delinquency[5].fromDay,
                tillDay: res?.data?.data?.delinquency[5].tillDay,
                promisesPerYear:
                  res?.data?.data?.delinquency[5].promisesPerYear,
                promisesPerLoan:
                  res?.data?.data?.delinquency[5].promisesPerLoan,
                penalty: res?.data?.data?.delinquency[5].penaltyAmount
                  ? res?.data?.data?.delinquency[5].penaltyAmount
                  : res?.data?.data?.delinquency[5].penaltyPercentage,
              });
              setradioInputValue(
  ...
  const updateSubmitForm: any = async () => {
    ...
    try {
      ...
      if (res.data.notificationMessage == "Operation successful.") {
        ...
        navigate("/lms/Setting/ProductManagement");
        setLoader(false);
      } else {
        toast.error(res.data.notificationMessage || res.data.errors[0]);
        setLoader(false);
      }
    } catch (error: any) {
      setLoader(false);
...
export default EditBrokenPromisses;
