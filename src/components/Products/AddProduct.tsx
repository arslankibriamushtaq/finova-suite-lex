...
const AddProduct = () => {
  ...
  const tapOptions = [
    ...
    //   key: "deliquency",
    //   folder: (
    //     <DeliquencyManagement tabs={setSelectedTab} productId={productId} />
    //   ),
    // },
  ];
  return (
...
function ProductNameTab({ tabs, setProductId }) {
  ...
  async function saveName() {
    ...
    try {
      ...
      await axios.post("/api/Product/AddProduct", payload).then((res) => {
        if (res.status == 200) {
...
function ProductSettingsTab({ tabs, productId }) {
  ...
  async function saveSettings() {
    ...
    try {
      ...
      const res = await axios.post("/api/Product/AddProductFinance", payload);
      if (res.data.notificationMessage == "Operation successful.") {
        toast.success(res.data.notificationMessage);
