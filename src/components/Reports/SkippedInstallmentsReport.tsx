...
const SkippedInstallmentsReport = () => {
  ...
  const handleSubmit = async () => {
    try {
      setLoading(true);
      const res = await getSkippedInstallmentsReport(
        fromDate || undefined,
        toDate || undefined
      );
      if (res) {
  ...
  const mappedData =
    ...
    allCallActivity.map((item: any) => {
      return {
        ...
        rescheduledDate: formatDate(item.rescheduledDate ? item.rescheduledDate : "_"),
...
export default SkippedInstallmentsReport;
